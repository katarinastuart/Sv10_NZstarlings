...
# Define input files ------------------------------------------------
vcffileALL <- "data/filtered_VCF/starling_noduprel_qual_miss_filt_bial_thin50kb.vcf.gz"
...
sfsdt <- plot_dt_SFS_compare_same_n_conf_only_missingsnps(glALL,
                                         popname_ls = poporder,
                                         iterations = 100,
                                         indmin = 10)

# sfsdt$median_quantile_plot_nLoc
save(sfsdt, file = "data/SFS_100_iterations_only_missingsnps_explore.Rdata")
load("data/SFS_100_iterations_only_missingsnps_explore.Rdata")
...
## Figure 6 ---------------------------------------------------------
pdf("results/SFS_ALL_thin50kb_only_missingsnps.pdf", width = 8.3, height = 3.5)
sfsdt$median_quantile_plot_nLoc + 
  facet_grid(.~popn) +
...
sfsdt <- plot_dt_SFS_compare_same_n_conf_nomissingsnps(glALL,
                                                          popname_ls = poporder,
                                                          iterations = 100,
                                                          indmin = 10)

# sfsdt$median_quantile_plot_nLoc
save(sfsdt, file = "data/SFS_100_iterations_nomissingsnps_explore.Rdata")
load("data/SFS_100_iterations_nomissingsnps_explore.Rdata")
...
dtcolshapes <- data.frame(pop = lab_order,
                          ...
                          shape = c(16, 3, 12, 18, 8, 14, 11, 17,15,7))
## Figure 6 ---------------------------------------------------------
pdf("results/SFS_ALL_thin50kb_nomissingsnps.pdf", width = 8.3, height = 3.5)
sfsdt$median_quantile_plot_nLoc + 
  facet_grid(.~popn) +
  scale_y_continuous(expand = expansion(mult = c(0, .02))) +
  theme(axis.text.x = element_text(angle = 90, colour = "black", size = 8, vjust = 0.5),
  ...
  xlab("Minor allele frequency (%)") +
  ylab("Median number of SNPs") +
  ggtitle("SFS plot, only SNPs with no missingness per pop (explore biases), subset n = 10, 100 iterations")
dev.off()
